...
class FileProcessor:
            ...
            for pos_pattern in positive_patterns:
                for component in path_components:
                    if pos_pattern in component:
                        ...
                        break # Encontró coincidencia para este patrón positivo
                if found_positive_match:
                    break # Encontró al menos una coincidencia positiva general
            if not found_positive_match:
                return False # Excluir si no coincide con ningún patrón positivo
    ...
    def procesar_archivos(config, incluir_ruta):
        ...
        positive_patterns = [p for p in all_patterns if not p.startswith('-')]
        ...
        for directorio_busqueda in rutas_de_busqueda:
            if not solo_archivos_especificos:
                for root_dir, dirs, files in os.walk(directorio_busqueda, topdown=True):
                    # --- Cambio: Aplicar patrones negativos a directorios ---
                    # Filtrar directorios prohibidos por config Y por patrón negativo ANTES de descender
                    dirs[:] = [
                        d for d in dirs
                        if d not in directorios_prohibidos_config and
                           FileProcessor._should_include(os.path.join(root_dir, d), positive_patterns, negative_patterns)
                    ]
                    # --- Fin Cambio ---
                    ...
                    for archivo in files:
                        ...
                        # --- Cambio: Aplicar patrones positivos/negativos al archivo/ruta ---
        ...
        for archivo_esp in archivos_especificos:
            nombre_archivo_objetivo = os.path.basename(archivo_esp) # Usar solo el nombre para buscar

            archivo_encontrado = None
            for directorio_busqueda in rutas_de_busqueda:
                 # Usar función _buscar_archivo_recursivo para buscar por nombre exacto (case-insensitive)
                 # en toda la estructura permitida (respetando directorios prohibidos por config)
                archivo_path_temporal = FileProcessor._buscar_archivo_recursivo(
                    directorio_busqueda,
                    nombre_archivo_objetivo,
                    directorios_prohibidos_config # Pasar directorios prohibidos por config
                )
            ...
            if archivo_encontrado:
                ...
                # --- Cambio: Aplicar patrones positivos/negativos al archivo específico encontrado ---
                ...
                archivos_no_encontrados.append(archivo_esp)
